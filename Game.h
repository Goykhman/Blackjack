 ...
 **************************************************************************
 ...
 %%      Game with one player and single deck using Basic Strategy       %%
 ...
 * number of rounds, where each round is played with the "round()" method.
 */
...
void Game::one_round(){
    ...
    if(player_natural){
        if(dealer_natural){
            //cout << "Both player and dealer have a natural.
            // Draw" << endl;
            draws+=1;
    ...
    while(true){
        ...
        if(player_soft&&soft_stand(player_count,dealer_hand[0])){
            //cout << "Soft hand stand" << endl;
            break;
        }
        // If player's hand is hard check whether it should stand.
        if(!player_soft&&hard_stand(player_hand.size(),player_count,
        ...
        // Check whether the hard hand should become soft (1) or the soft
        // hand should become hard (2):
        // 1. Check whether the dealt card is an ace which is the first
        // ace in player's hand and can be counted as 11, making the
        // hard hand soft.
        if(rank=='A'){
            vector<char>::iterator it=find(player_hand.begin(),
                                           player_hand.end(),'A');
            if(it==player_hand.end()&&player_soft==false&&
                                           player_count+11<=21){
                player_soft=true;
                val+=10;
            }
        }
        // 2. Check whether player's hand is soft but should become hard.
        // If hand is currently soft there should be exactly one ace.
        else if(player_soft&&player_count+val>21){
            //cout << "Hand was soft but becomes hard" << endl;
            player_soft=false;
            // Recalculate the value of the new card.
            val=value(rank,player_soft);
            // The hand was soft, meaning there was exactly one ace.
            // Now it's hard, hence the count is less by 10.
            player_count-=10;
        }
    ...
    while(true){
        ...
        // hand should become hard (2):
        // 1. Check whether the dealt card is an ace which is the first
        // ace in dealer's hand and can be counted as 11, making the
        // hard hand soft.
        if(rank=='A'){
            vector<char>::iterator it=find(dealer_hand.begin(),
                                           dealer_hand.end(),'A');
            if(it==dealer_hand.end()&&dealer_soft==false&&
               dealer_count+11<=21){
                dealer_soft=true;
                val+=10;
            }
        }
        // 2. Check whether dealer's hand is soft but should become hard.
        // If hand is currently soft there should be exactly one ace.
        else if(dealer_soft&&dealer_count+val>21){
            //cout << "Hand was soft but becomes hard" << endl;
            dealer_soft=false;
            // Recalculate the value of the new card.
            val=value(rank,dealer_soft);
            // The hand was soft, meaning there was exactly one ace.
            // Now it's hard, hence the count is less by 10.
            dealer_count-=10;
        }
...
void Game::split_round(const char & player_rank){
    ...
    if(player_natural){
        if(dealer_natural){
            //cout << "Player and dealer even at natural" << endl;
            draws+=1;
            player_time_series.push_back(player_bankroll);
            return;
        }
    ...
    if(player_count>dealer_count){
        ...
        //" higher than the dealer count " << dealer_count << endl;
        player_won+=1;
        player_bankroll+=bet_size;
        dealer_bankroll-=bet_size;
        times_player_split_and_won++;
...
// Reshuffling happens either after we go through 2/3 of the deck
// or at a random time between A and B since the last reshuffle.
void Game::play(const int & rounds, const int & A, const int & B){
    
    // Initialize the random reshuffle time "rand"
    int rand=A+rand_index(B-A);
    ...
    while(rounds_played<rounds){
        ...
        if(get_pointer()>get_order().size()/3){
            ...
            rand=rounds_played+A+rand_index(B-A);
            //cout << "rand = " << rand << endl;
        }
        
        // Check whether it's a time for a random reshuffle and if yes
        // reshuffle and set the new random reshuffle time "rand" to
        // A+rand_index(B-A) from the current round
        else if(rounds_played==rand){
            reset();
            rand=rounds_played+A+rand_index(B-A);
            //cout << "Reset pointer = " << get_pointer() << endl;
            //cout << "rand = " << rand << endl;
